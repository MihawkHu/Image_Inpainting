...
def get_patch(x, y, img, patch_size):
    p = patch_size // 2
    ...
    patch = img[x_begin : x+p+1, y_begin : y+p+1]
...
def get_exemplar_patch(patch, x, y, img, patch_size):
    p = patch_size // 2
    x_boundary = img.shape[0]
    y_boundary = img.shape[1]
    
    # source patch that will copy from
    img_copy = img[p : x_boundary-p+1, p : y_boundary-p+1]
    ...
    min_ssd = np.inf
    while i < len(xx) - 1:
        exemplar_patch = get_patch(xx[i] + p, yy[i] + p, img, patch_size)
...
def update_patch(x, y, patch, img, patch_size):
    p = patch_size // 2
